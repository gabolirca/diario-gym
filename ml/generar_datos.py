    ...
    for semana in range(semanas):
        ...
        for d in dias_entreno:
            ...
            for idx in hoy:
                ...
                # Adaptación con rendimientos decrecientes sobre el potencial actual
                st["pot"] += mult * impulso * 0.55 * np.exp(-st["pot"] / (peso_actual * 2.6))
                st["ultimo_dia"] = dia
                ...
                elif rir_obs < rmin:                    # se quedó corto
                    paso = -1
                else:
                    paso = 1 if rng.random() < 0.45 else 0
                if rng.random() < 0.07:                 # despiste: repite o salta
