-- =====================================================================
-- GymAI · 15 · Vistas del panel de seguimiento
-- Todas con security_invoker: si quien consulta no es investigador,
-- solo verá su propia fila. La anonimización no depende del cliente.
-- =====================================================================

-- Una fila por participante, con todo lo que decide si el estudio vive
create or replace view public.v_admin_participantes as
with base as (
  select p.id, p.display_name, p.sex, p.goal, p.training_days_per_week,
         ...
         p.research_consent, p.onboarded_at, p.created_at,
...
select
  ...
  -- Semanas transcurridas desde el inicio de su plan
...
left join lateral (
  select count(distinct wo.id)                                as sesiones,
         ...
         count(ws.rir)                                        as series_con_rir
...
comment on view public.v_admin_participantes is
  'Seguimiento del estudio. pct_rir es la métrica crítica: sin RIR la serie no sirve para entrenar el modelo.';

-- ---------------------------------------------------------------------
-- Progreso semanal por participante, para las gráficas
create or replace view public.v_admin_progreso as
select
  w.user_id,
  greatest(1, floor((w.performed_on - p.plan_start)/7.0)::int + 1) as semana,
  count(distinct w.id)                          as sesiones,
  round(sum(ws.volume_kg)/1000.0, 2)            as tonelaje_t,
  round(avg(ws.e1rm_kg), 1)                     as e1rm_promedio,
  round(avg(ws.rir), 2)                         as rir_promedio,
  count(ws.id)                                  as series,
  count(ws.rir)                                 as series_con_rir
...
group by w.user_id, greatest(1, floor((w.performed_on - p.plan_start)/7.0)::int + 1);

alter view public.v_admin_progreso set (security_invoker = on);

-- ---------------------------------------------------------------------
-- Progreso por ejercicio: la curva que muestra si alguien se hace fuerte
create or replace view public.v_admin_ejercicios as
select
  es.user_id,
  e.name                    as ejercicio,
  e.slug,
  count(*)                  as sesiones,
  min(es.performed_on)      as desde,
  max(es.performed_on)      as hasta,
  round(min(es.top_e1rm_kg), 1) as e1rm_inicial,
  round(max(es.top_e1rm_kg), 1) as e1rm_maximo,
  round(avg(es.avg_rir), 2)     as rir_promedio
from public.v_exercise_sessions es
join public.exercises e on e.id = es.exercise_id
group by es.user_id, e.name, e.slug;

alter view public.v_admin_ejercicios set (security_invoker = on);
